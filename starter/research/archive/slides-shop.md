...
info: A 20-minute talk about pstack through an illustrative online-shop feature and bug fix.
...
<img class="cover-art" src="/covers/learning-from-pstack.png" alt="Storybook cover for Learning from Pstack, by Alexander Opalic." />
...
<img class="cover-art" src="/illustrations/feature-factory.png" alt="An automated feature factory promises a finished feature at the press of a button." />
...
<div class="steps"><div>1. The agent can verify its changes</div><div>2. Clear engineering guidance</div><div>3. A workflow that fits the task</div></div>
...
<p class="sub">Skills · playbooks · principles</p>
...
<img class="cover-art" src="/illustrations/poteto-profile.png" alt="Profile screenshot of Lauren, known as poteto, the creator of pstack." />
...
# Our shop needs discount codes

<img class="diagram-image" src="/diagrams/shop-shop.svg" alt="A €100 basket with SAVE10 applied has a €10 discount and a €90 total." />
...
<img src="/illustrations/poteto-mode-entry.png" alt="Prompt composer with poteto-mode selected." class="entry-image" />

<pre class="plain-code small">Add discount codes to checkout.
Recalculate when quantities change.
Verify the customer journey. Keep changes local.</pre>
...
<p class="source">Selected lines; description omitted · skills/how/SKILL.md</p>
...
# How
```
<p class="sub">Metadata identifies it.<br>The body tells the agent how to work.</p>
...
# The request selects a playbook

<img class="diagram-image" src="/diagrams/shop-route.svg" alt="Poteto-mode routes a request to Feature or Bug fix." />

<p class="source">Exact routing entries, line-wrapped · skills/poteto-mode/SKILL.md</p>

```md
- **Feature.** New or changed behavior,
  built from a named data shape. `playbooks/feature.md`.
- **Bug fix.** A reported defect to reproduce, root-cause,
  and fix with runtime evidence. `playbooks/bug-fix.md`.
```
...
# Understand the checkout first

<img class="diagram-image" src="/diagrams/shop-understand.svg" alt="Trace cart items and code through pricing rules to checkout display and submission." />

<p class="source">Exact first step · playbooks/feature.md</p>
...
# Compare designs before implementation

<img class="diagram-image" src="/diagrams/shop-design.svg" alt="Compare checkout-owned discount calculation with pricing-owned calculation before choosing." />

<p class="source">architect → competing design sketches → comparison</p>
...
# Make the discount state explicit

<img class="diagram-image" src="/diagrams/shop-domain.svg" alt="A discount result is applied with an amount or rejected with a reason, and feeds one pricing result." />

<p class="source">Exact principle sentence · principle-model-the-domain/SKILL.md</p>
...
<img class="diagram-image" src="/diagrams/shop-delegate.svg" alt="Lead assigns scope and checks, worker implements, and lead reviews the actual diff." />

<p class="source">Illustrative assignment · Example worker assignment</p>

```md
Scope: pricing module and checkout callers.
Model: DiscountResult and PricingResult.
Check: quantity changes recalculate discounts.
Return: diff, verification, open questions.
```
...
<img class="diagram-image" src="/diagrams/shop-verify.svg" alt="Apply SAVE10 for €90, change quantity to three for €135, then remove the code for €150." />

<p class="source">Prove It Works · expected behavior in this example</p>
...
# Later, a checkout regression

<img class="diagram-image" src="/diagrams/shop-bug.svg" alt="After a later checkout change, applying SAVE10 works but changing quantity shows €150 instead of €135." />

<p class="source">New request → poteto-mode → Bug fix</p>
...
<p class="source">Exact opening sentence, line-wrapped · playbooks/bug-fix.md</p>

```md
1. Reproduce it yourself on the matching surface
   via the control skill (Non-negotiables).
```
<div class="steps"><div>Two totes → apply SAVE10 → €90</div><div>Quantity 3 → observe €150</div><div>Expected €135</div></div>
...
# Eliminate causes with evidence

<img class="diagram-image" src="/diagrams/shop-cause.svg" alt="Check whether the code was cleared, pricing was skipped, or the display was stale, then confirm one mechanism." />

<p class="source">how · current behavior &nbsp;&nbsp; why · regression history</p>
...
# Fix the cause and repeat the repro

<img class="diagram-image" src="/diagrams/shop-fix.svg" alt="Quantity changes call shared pricing with items and code, producing the expected €135 total." />

<p class="source">Example outcome to verify · €150 before → €135 after</p>
...
# Same shop, different starting evidence

<img class="diagram-image" src="/diagrams/shop-compare.svg" alt="Feature begins with understanding and design; Bug fix begins with reproduction and diagnosis. Both include review and verification." />

<p class="source">Simplified comparison · delivery steps omitted</p>
...
<pre class="plain-code">Use poteto-mode.
Implement [one concrete behavior].
Follow this project’s conventions.
Verify the real user journey.
Keep changes local.</pre>
<p class="sub">Read the instructions. Inspect the result.</p>
...
<p class="rule">Which instruction would help<br>your agent most?</p>
<p class="sub">Your investigation, design, or verification workflow</p>
...
<h1 class="cover-accessible-title">Computer handoff example</h1>
<img class="cover-art" src="/illustrations/sales-outbound-computer.png" alt="Sales Outbound assistant requests a Salesforce sign-in through a computer handoff." />
...
# A real Starfall verification finding

<img class="screenshot" src="/evidence/flame-paused-fixed.png" alt="Recorded corrected paused Starfall state after selling and rebuilding a flamethrower." />
...
# Source files behind the diagrams

<div class="steps small"><div>skills/poteto-mode/SKILL.md</div><div>playbooks/feature.md · playbooks/bug-fix.md</div><div>skills/how · skills/architect</div><div>Model the Domain · Prove It Works</div></div>
<p class="source">cursor/plugins · 6ed0f7a · illustrative shop scenarios</p>
